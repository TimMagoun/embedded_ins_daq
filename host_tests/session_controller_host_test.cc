...
TEST(SessionControllerTest, StartsRecordingAutomaticallyOnBootWhenConfigValid) {
  ...
  ASSERT_EQ(session_controller_mark_storage_ready(&controller), ESP_OK);
  ASSERT_EQ(session_controller_mark_config_loaded(&controller, &config),
            ESP_OK);
  ASSERT_EQ(session_controller_start_autonomously(&controller), ESP_OK);
