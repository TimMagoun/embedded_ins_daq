...
- **Milestone 1:** Core runtime, UART capture, clock, and SYNC association.
...
- Monotonic clock, RTC fallback, GNSS discipline, and clock transition/drift logging.
...
### Goal
Deliver a minimum viable firmware that can acquire sensor data from the UART ports, timestamp it against the shared clock, associate packets with SYNC events, and expose enough runtime status to validate capture quality.

### Deliverables
- Firmware startup path with the planned task split between real-time capture work and background services.
- Shared monotonic microsecond clock available to all subsystems.
- Clock source selection at startup using GNSS SYNC, external RTC, or free-running fallback.
- Clock drift measurement and clock-source transition reporting.
...
- Session/runtime status reporting over console or internal status output for port health, parser state, clock source, and error counters.
...
- Confirm whether the device is using GNSS SYNC, RTC, or free-running time.
...
- GNSS-present, RTC-only, and free-running startup cases all select the expected clock source.
- Drift records and clock-source transition records are generated when source conditions change.
...
- Unique file naming using wall-clock time when available and monotonic fallback when not.
...
### User should be able to
- Run a session that captures UART data, sync events, clock status, and trigger events into SD log files.
...
- The binary log contains the published header and record structure needed for an external reader.
...
### Deliverables
- Startup configuration loading from `/sd/config.json` covering ports, clock, trigger, network, and NTRIP behavior.
...
- HTTP status endpoint for device state, uptime, clock source, and per-port statistics.
