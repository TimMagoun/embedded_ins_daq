...
- **REQ-SYNC-03:** The firmware shall associate each incoming UART packet with a SYNC event at capture time, based on a configured nominal packet rate for the port. If a packet is not received within the expected window, the firmware shall resynchronize the association sequence and log the gap.
...
- **REQ-CLK-02:** The clock source shall follow this priority order:
  ...
  |----------|--------|-----------|
  | 1 | GNSS SYNC pulse | A port is designated as clock master and a consistent periodic SYNC input is present |
...
- **REQ-CLK-03:** When locked to a GNSS SYNC signal, the device shall correct clock drift using slew-rate adjustment — the clock shall never jump backward. The maximum slew rate shall be configurable.
...
- **REQ-SD-01:** The device shall log all data to SD card (SDHC/SDXC, 32 GB or larger). SD writes shall not stall sensor acquisition, timestamping, or UART reception.
...
- **REQ-NET-06:** Unstreamed data shall be retained in memory during an outage. The buffer shall hold up to 500 KB per interface. Once full, the oldest unstreamed data shall be overwritten and a gap event logged. Data on the SD card shall never be deleted or overwritten.
