...
- **CRITICAL HARDWARE CONSTRAINT:** Silicon v1.3 uses a legacy memory mapping compared to production v3.0+ chips. You MUST ensure `CONFIG_ESP32P4_SELECTS_REV_LESS_V3=y` and `CONFIG_ESP32P4_REV_MIN_103=y` are enforced in `sdkconfig.defaults`. Do not bypass bootloader revision checks without explicitly alerting the user.
...
- **Strict Tooling:** Never use raw `gcc`, `make`, or direct `python` commands. Always use the official build system:
  - Build: `idf.py build`
  - Clean: `idf.py fullclean`
  - Monitor/Flash: `idf.py flash monitor`
...
- **Unit Testing:** Use the Unity test framework built into ESP-IDF. Place test files in the `test/` directory and ensure tests can run via `idf.py test`.
