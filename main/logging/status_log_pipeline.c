#include "status_log_pipeline.h"

#include <string.h>

#include "log_pipeline_base.h"

static log_pipeline_base_t status_log_pipeline_to_base(
    const status_log_pipeline_t* pipeline) {
  log_pipeline_base_t base = {};
  if (pipeline == NULL) {
    return base;
  }

  base.storage = (uint8_t*)pipeline->storage;
  base.capacity_bytes = pipeline->capacity_bytes;
  base.pending_bytes = pipeline->length;
  return base;
}

static void status_log_pipeline_apply_base(status_log_pipeline_t* pipeline,
                                           const log_pipeline_base_t* base) {
  if (pipeline == NULL || base == NULL) {
    return;
  }

  pipeline->storage = (char*)base->storage;
  pipeline->capacity_bytes = base->capacity_bytes;
  pipeline->length = base->pending_bytes;
}

static void status_log_pipeline_terminate(status_log_pipeline_t* pipeline) {
  if (pipeline != NULL && pipeline->storage != NULL &&
      pipeline->capacity_bytes > 0U) {
    pipeline->storage[pipeline->length] = '\0';
  }
}

void status_log_pipeline_init(status_log_pipeline_t* pipeline, char* storage,
                              size_t capacity_bytes) {
  log_pipeline_base_t base = {};
  log_pipeline_base_init(&base, storage, capacity_bytes);
  status_log_pipeline_apply_base(pipeline, &base);
  status_log_pipeline_terminate(pipeline);
}

esp_err_t status_log_pipeline_append(status_log_pipeline_t* pipeline,
                                     const char* message) {
  const size_t message_length = message == NULL ? 0U : strlen(message);
  log_pipeline_base_t base = {};

  if (pipeline == NULL || pipeline->storage == NULL || message == NULL) {
    return ESP_ERR_INVALID_ARG;
  }

  base = status_log_pipeline_to_base(pipeline);
  if (message_length > 0U) {
    const esp_err_t status =
        log_pipeline_base_append(&base, message, message_length);
    if (status != ESP_OK) {
      return status;
    }
  }

  if (log_pipeline_base_append(&base, "\n", 1U) != ESP_OK) {
    return ESP_ERR_NO_MEM;
  }

  status_log_pipeline_apply_base(pipeline, &base);
  status_log_pipeline_terminate(pipeline);
  return ESP_OK;
}

esp_err_t status_log_pipeline_flush(status_log_pipeline_t* pipeline, char* out,
                                    size_t out_capacity,
                                    size_t* bytes_written) {
  log_pipeline_base_t base = status_log_pipeline_to_base(pipeline);
  const esp_err_t status =
      log_pipeline_base_flush(&base, out, out_capacity, bytes_written);
  status_log_pipeline_apply_base(pipeline, &base);
  status_log_pipeline_terminate(pipeline);
  return status;
}

size_t status_log_pipeline_pending_bytes(
    const status_log_pipeline_t* pipeline) {
  const log_pipeline_base_t base = status_log_pipeline_to_base(pipeline);
  return log_pipeline_base_pending_bytes(&base);
}
